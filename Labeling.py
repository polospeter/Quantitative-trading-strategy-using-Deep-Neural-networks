...
#        if ts.index[i]>k and ts.index[i]<l:
    ...
    # it should end with the sell signal or not??
...
def transformtocontinuos(labels):
    ...
    for i in range(2,len(labels)-2):
        #what if the first two index or the last two is the signal?? deal with it later
        if labelss[i]==1:
            newlabels[i-1]=0.7
            newlabels[i+1]=0.7
            newlabels[i+2]=0.3
...
# Function Buy finds the first optimal buy index in the series of prices x starting from the index buy id,
...
# Function Sell finds the first optimal sell index in the series of prices x starting from the index sell id
...
# One from the PR.investor paper 
...
def buyandwait(x_test,y_test):
    ...
    return tra,df # maybe give some name to the column
...
def plrlabeling(stock,threshold):
    ...
    # Right now I do not need to plot it!
...
# Clear up consecutive labels and gaurantees that only profitable trade takes place
